...
out.dir <- file.path(this.dir, "_ssd") # output directory
...
load(file.path(this.dir, "phd_v1",      "summary", "eval.RData")); phd <- eval;      remove(eval);
...
for (s in levels(factor(pnr$S))) { ## for each S  levels(factor(pnr$S))
  ...
  val <- do.call(rbind, lapply(split(t0, t0[,"NAME"]), function(x) x[which.min(x[,"SSD"]),c("NAME","SSD")]))
  names(val)[names(val)=="SSD"] <- "APP2"
  
  cat("APP2 = ", nrow(val), "\n")
  
  t0 <- gps[gps$S %in% s,]
  t <- do.call(rbind, lapply(split(t0, t0[,"NAME"]), function(x) x[which.min(x[,"SSD"]),c("NAME","SSD")]))
  names(t)[names(t)=="SSD"] <- "GPS"
  ...
  ##################################################
  file_name <- file.path(out.dir, paste("ssd_s=", format(as.double(s), nsmall=1, decimal.mark="_"),".pdf", sep=""))
  pdf(file_name, width=4, height=4, useDingbats=F, family="Times")
  #par(mar=c(2,3.5,.8,.4)+0.1, mgp=c(1.2, 0.5, 0))
  ...
  bp<-boxplot(val$APP2, val$GPS, val$MST, val$PHD, val$PNR, 
    ...
    xaxt = "n",  xlab = "", ylab= "SSD", ylim=c(0,120))
  ...
  # text(xmin-1.3, (ymin+ymax)*.5, labels=c("SSD"), pos=4, xpd=T, cex=1.3)
...
##################################################
file_name <- file.path(out.dir, paste("ssd_med.pdf", sep=""))
pdf(file_name, width=4, height=4, useDingbats=F, family="Times")
par(mar=c(2.5,3.5,.2,.2)+0.1, mgp=c(1.6, 0.5, 0))
plot( med.s$S, med.s$MST,    type="o", lty=1, lwd=1,  pch= pch_mst, col=col_mst, 
      ylab =NA, xlab="S", ylim=c(ymn,ymx+0.25*(ymx-ymn)), xaxt="n", yaxt="n", cex=1.5, cex.lab=1.3)
...
text(xmin-.95, (ymin+ymax)*.5, labels=c("SSD"), pos=4, xpd=T, cex=1.3)
legend("topleft",
...
ymn<-min(avg.s[,-1]) - 3.0 # shift bottom to fit the legend
ymx<-max(avg.s[,-1])

##################################################
file_path <- file.path(out.dir, paste("ssd_avg.pdf", sep=""))
pdf(file_path, width=4, height=4, useDingbats=F, family="Times")
par(mar=c(2.5,3.5,.2,.2)+0.1, mgp=c(1.6, 0.5, 0))

plot( avg.s$S, avg.s$MST,    type="o", lty=1, lwd=1,  pch= pch_mst,  col=col_mst,
      ylab = "SSD", xlab = "S", ylim=c(ymn,ymx+0.50*(ymx-ymn)), xaxt="n", yaxt="n", cex=1.5, cex.lab=1.3)
