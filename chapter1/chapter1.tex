...
\begin{equation}
C(\mathrm{X}) = \int_{\mathrm{X}} c( \mathrm{x}_i, I(\mathrm{x}) ) d\mathrm{x}
\label{ch1_eq3}
\end{equation}
...
\begin{equation}
c( \mathrm{x}, I(\mathrm{x}) ) = \gamma c_{\text{G}}( \mathrm{x} ) + (1 - \gamma) c_{\text{I}}( \mathrm{x}, I(\mathrm{x}) ), \; \; \gamma \in \left[ 0, 1 \right]
\end{equation}

where $ \gamma \in \left[ 0, 1 \right] $. Intensity-based component can directly use the sampled/interpolated voxel intensities $I(x, y, z)$ or their normalized values $\tilde{I}(x,y,z) = \frac{I}{I_{\text{max}}} $ where $ c_{\text{I}}(x, I(\mathrm{x})) = e^{\lambda (1-\tilde{I}(\mathrm{x}))^2} $ or the tubularity measure $\rho$ derived from the ratios of the eigen values computed at different scales $ c_{\text{I}}() = \rho(\mathrm{p}) = \rho( (x, y, z) ) $ \cite{sato19973d,meijering2004design,frangi1998multiscale}.
