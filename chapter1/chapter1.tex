...
% In decades following the emergence of neuroscience, the instrumentation has been a limiting factor to the amount and the variety of the morphologic data that could be gathered \ref{ch1_fig1}. At the beginning of the second half of the 20th century, conventional electronic analog techniques were used to join microscope and the analogue computer to automate the neuron morphology quantification \cite{glaser1965semi,capowski1981accurate}.
...
The key obstacles in reaching the accurate and robust automation of the neuron reconstruction \cite{meijering2010neuron,donohue2011automated,acciai2016automated} can be seen through the number of hampering factors that concern both the intricate nature of the data and the barriers imposed by the imaging modality. In this regard, it is possible to identify some major hurdling items.

\textit{Morphology} of the neuron cell is remarkably diverse (Fig.~\ref{ch1_fig7}). The structural intricacy of the very complex examples can be intractable even to the human visual comprehension. High complexity requires large amount of hours needed for manual delineation. The delineation thus inevitably becomes more error-prone. Dealing with challenging morphology requires novel algorithmic computer vision solutions. The aim of methods is to reduce the manual labor time while fabricating trustful reconstructions. For instance, the 20-fold speedup of the reconstruction time compared to the manual reconstruction had been projected in earlier challenges \cite{liu2011diadem}.

\textit{Imaging} objects at $\mu m$ scale size using fluorescence or other kinds of microscopy imposes accuracy limitations.  which are specific for the microscopy. Even if one would have flawless ideal lenses, certain limitation . With the PSF effect, much of the detailed structure can be lost in 

- optical
- digitization (pixelization) effects 


 Namely, is \cite{shaw1991point}
noisy image input (imaging on the boundary of the resolution limits in case of the light microscopy).  Moreover, there are limitations imposed by microscopy. Moreover, particles and debris can be erroneously detected as neuron structures. Due to the sample preparation and the imaging technique, spatial resolution differs along z-axis (layers) compared to the one existing in the image plane (xy). Fluorescence or various kinds of light microscopy modalities. The lateral $(xy)$ and axial $(z)$ resolutions are different. The resolution details are recorded in the 

\textit{Data volume size} ever increasing size of the neuronal image stacks \cite{halavi2012digital} needs to be correctly processed in reasonable time. Although the processing of the 3D data (image stacks) is a  Processing of the stitched images.
...
Bayesian filtering \cite{doucet2001introduction} offers a common generic solution for problems that are non-linear, where state-space cannot be Gaussian. Other names for the same concept are: (optimal nonlinear, stochastic). In essence, the approach boils down to computing posterior distributions of the values intended to be filtered. Basic algorithm applicable in different contexts. The problem is defined as the estimate the set of hidden states
...
$$ \{ \mathrm{x}_t, t \in \mathbb{N} \} $$
$$ \{ \mathrm{y}_t, t \in \mathbb{N} \} $$
...
The thesis is organized as follows. In Chapter 2,  
